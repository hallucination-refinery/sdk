...
**What This Proves**: The Refinery SDK delivers deterministic, performant graph visualization with attribute-only lens transitions, maintaining 60fps at 1k nodes without physics simulation.
...
### 30-Second Journey
1. **0-5s**: Page loads, glowing wireframe brain with concept neurons mapped to surface
...
- **Affinity**: Category → color hue (8 distinct)
- **Temporal**: Recency → brightness (0.3-1.0 opacity)
- **Causal**: Selected paths → pulse animations

### Edge Policy
- **Default**: OFF (performance + clarity)
- **On Selection**: Show edges to connected concepts
...
- **Secret handling**: Dim (0.5x opacity), reduce size (0.7x)
...
- `DetailsPanel`: Concept info + memory list
...
- **Demo**: Complete experience at 60fps
...
- **Consecutive temporal** (not 7-day window)
...
### M0: Brain Canvas + Minimal HUD + Tiny Fixture (Days 1-5)

#### Day 1: Scene Setup
- [ ] Initialize Three.js with R3F (`npm create vite@latest`)
- [ ] Configure WebGLRenderer (antialias, alpha)
- [ ] Add OrbitControls with constraints
- [ ] Setup stats.js FPS meter
- [ ] Verify 60fps baseline

#### Day 2: Brain Mesh Integration
- [ ] Load brain.obj via useLoader
- [ ] Apply wireframe material (emissive: #00ffff, opacity: 0.6)
- [ ] Extract vertices to TypedArray (~10k expected)
- [ ] Implement region classification (frontal/temporal/parietal/occipital)
- [ ] Store vertex positions for mapping

#### Day 3: Particle System
- [ ] Create instanced BufferGeometry for concepts
- [ ] Write vertex shader with position/size/color attributes
- [ ] Write fragment shader with glow effect
- [ ] Test with 100 random particles on brain surface
- [ ] Implement raycasting for hover detection

#### Day 4: Enriched Fixture
- [ ] Create sample memory dataset (30 memories)
- [ ] Generate concept extraction (10-15 concepts)
- [ ] Add `createdFrom` arrays linking to memories
- [ ] Calculate edge weights for all lens types
- [ ] Save as `dataset-enriched.json`

#### Day 5: HUD Shell & Integration
- [ ] Create React component structure
- [ ] Position HUD with CSS Grid overlay
- [ ] Add disabled lens selector buttons
- [ ] Load fixture and render concepts on brain
- [ ] **M0 Demo**: Brain with test concepts at 60fps

### M1: Data Integration (Days 6-10)

#### Day 6: Store Architecture
- [ ] Setup Zustand with concept/lens/selection slices
- [ ] Implement memory provenance slice
- [ ] Add filter/search actions
- [ ] Connect store to React components
- [ ] Test state updates trigger re-renders

#### Day 7: Enrichment Service
- [ ] Create `/api/enrich` endpoint (Connected strict)
- [ ] Implement `POST /enrich` with memories[] → concepts
- [ ] Add same-origin validation
- [ ] Fallback to pre-enriched dataset.json
- [ ] Handle timeout/error states

#### Day 8: Vertex Mapping
- [ ] Implement deterministic hash function
- [ ] Create concept→vertex assignment
- [ ] Handle collisions with spiral search
- [ ] Add shell expansion (1.01x, 1.02x) for overflow
- [ ] Test with varying concept counts

#### Day 9: Selection System
- [ ] Implement raycaster picking
- [ ] Add selection highlighting (scale 1.5x)
- [ ] Create details panel component
- [ ] Display concept label + memory list
- [ ] Add keyboard navigation

#### Day 10: Integration Test
- [ ] Full flow: Load→Enrich→Map→Select
- [ ] Performance profiling
- [ ] Memory leak detection
- [ ] **M1 Demo**: Concepts with memory provenance

### M2: Lens System (Days 11-15)

#### Day 11: Affinity Lens
- [ ] Create category→HSL color mapping (8 hues)
- [ ] Update particle color uniforms
- [ ] Implement smooth transitions (300ms)
- [ ] Add category legend to HUD
- [ ] Test all categories distinct

#### Day 12: Temporal Lens
- [ ] Calculate recency scores (0.3-1.0 brightness)
- [ ] Update particle brightness uniforms
- [ ] Create timeline component
- [ ] Add date range filtering
- [ ] Implement play animation

#### Day 13: Causal Lens
- [ ] Implement causal path detection (heuristic)
- [ ] Add pulse shader uniforms
- [ ] Create pulse animations (1s cycle)
- [ ] Highlight selected paths only
- [ ] Add path explanation in details

#### Day 14: Lens Switching
- [ ] Enable lens selector buttons
- [ ] Implement attribute-only updates (<800ms)
- [ ] Verify NO position recalculation
- [ ] Add smooth attribute transitions
- [ ] Update HUD for each lens

#### Day 15: Lens Testing
- [ ] Test all three lenses
- [ ] Verify attribute-only changes
- [ ] Profile switching performance
- [ ] **M2 Demo**: Lenses without position changes
...
- [ ] **M3 Demo**: Production-ready at 60fps
