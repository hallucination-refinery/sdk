...
| Vertex position logging            | Log sample `revealPos` / `gl_Position` to confirm geometry collapse.        | codex/instrument-vertex-positions-for-debugging (PR #245) | In review   | `[vertex] samples` logs must be captured per the new telemetry. |
