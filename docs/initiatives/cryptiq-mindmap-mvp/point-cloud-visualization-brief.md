...
## CURRENT STATUS (2025-08-29, approx.)
...
- Debug path currently committed to bypass unprojection and draw directly in clip space from NDC to validate attributes; despite this, user still sees a dot/black screen.
...
2) Minimal render path (no custom shaders).
...
3) Restore custom shader gradually.
...
5) Performance safeguards.
   - Cap points to ≤150k initially; `stride`≥2.
...
- Are color/depth images the same dimensions at runtime? We min() them but mismatches may zero out sampling.
