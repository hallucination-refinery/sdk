...
Ultrathink, observe, orient, decide and execute: **add a pre-merge QA checklist to every pull request (task qa-5)** by creating `.github/PULL_REQUEST_TEMPLATE.md` with coverage, perf, evidence and CI tick-boxes, wiring a CI action or danger script to block merges if any item is unchecked, and committing as `chore(ci): add QA PR checklist (task qa-5)`.
