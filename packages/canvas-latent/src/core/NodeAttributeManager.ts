...
export class NodeAttributeManager {
  ...
  private initializeColorPriorities(): void {
    ...
    this.colorPriorities.set('category:secondary', {
  ...
  setMesh(mesh: THREE.InstancedMesh): void {
    this.mesh = mesh
    // Initialize updateRange/usage defensively for all attributes we may touch
    const instMatrix: any = (mesh as any).instanceMatrix
    if (instMatrix) {
      if (!instMatrix.updateRange) instMatrix.updateRange = { offset: 0, count: -1 }
      if (typeof instMatrix.setUsage === 'function') instMatrix.setUsage(THREE.DynamicDrawUsage)
      else instMatrix.usage = THREE.DynamicDrawUsage as any
    }
    const instColor: any = (mesh as any).instanceColor
    if (instColor) {
      if (!instColor.updateRange) instColor.updateRange = { offset: 0, count: -1 }
      if (typeof instColor.setUsage === 'function') instColor.setUsage(THREE.DynamicDrawUsage)
      else instColor.usage = THREE.DynamicDrawUsage as any
    }
    const aOpacityAttr: any = mesh.geometry?.getAttribute?.('aOpacity')
    if (aOpacityAttr) {
      if (!aOpacityAttr.updateRange) aOpacityAttr.updateRange = { offset: 0, count: -1 }
      if (typeof aOpacityAttr.setUsage === 'function') aOpacityAttr.setUsage(THREE.DynamicDrawUsage)
      else aOpacityAttr.usage = THREE.DynamicDrawUsage as any
    }
  }
  ...
  flush(): void {
    ...
    if (this.dirtyRangeMatrix.min !== Infinity) {
      const instMatrix: any = (this.mesh as any).instanceMatrix
      if (instMatrix && instMatrix.array) {
        const start = this.dirtyRangeMatrix.min * 16
        const count = (this.dirtyRangeMatrix.max - this.dirtyRangeMatrix.min + 1) * 16
        instMatrix.array.set(this.instanceMatrix.subarray(start, start + count), start)
        if (instMatrix.updateRange) {
          instMatrix.updateRange.offset = start
          instMatrix.updateRange.count = count
        } else {
          // Fallback to full update if updateRange unsupported/undefined
          instMatrix.needsUpdate = true
        }
        instMatrix.needsUpdate = true
      }
    }
    ...
    if (this.dirtyRangeColor.min !== Infinity) {
      ...
      const colorAttr: any = (this.mesh as any).instanceColor
      if (colorAttr && colorAttr.array) {
        colorAttr.array.set(this.instanceColor.subarray(start, start + count), start)
    ...
    if (this.dirtyRangeOpacity.min !== Infinity) {
      ...
      const opacityAttr: any = this.mesh.geometry?.getAttribute?.('aOpacity')
      if (opacityAttr && opacityAttr.array) {
        opacityAttr.array.set(this.aOpacity.subarray(start, start + count), start)
