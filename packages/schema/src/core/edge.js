 ...
 */
export function edgeConnects(edge, nodeId1, nodeId2) {
    return ((edge.source === nodeId1 && edge.target === nodeId2) ||
        (!edge.directed && edge.source === nodeId2 && edge.target === nodeId1));
}
/**
 * Get the other node ID from an edge given one node ID
 */
export function getOtherNode(edge, nodeId) {
