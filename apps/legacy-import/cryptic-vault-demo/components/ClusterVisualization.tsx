...
function ClusterMetaball({ cluster, opacity }: { cluster: Cluster; opacity: number }) {
  const meshRef = useRef<THREE.Mesh>(null)
  const { activeCategories } = useCategory()

  // Check if this cluster is active
  const isActive =
    !activeCategories || activeCategories.size === 0 || activeCategories.has(cluster.id)
  const finalOpacity = isActive ? opacity * 0.7 : opacity * OPACITY_VALUES.dimmed
