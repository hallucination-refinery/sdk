...
export default function CrypticAnimusScene({
  ...
  if (data.nodes.length > 0) {
    const node = data.nodes.find((n) => n.id === 'c_001')
    console.log(
      `[Step E] Is node c_001 frozen on RECEIVING props in AnimusScene?`,
  ...
  const {
    ...
    // Deep-clone the incoming data to ensure it's mutable for d3-force
    const nodes =
  ...
  const nodePassesFilters = useCallback(
    (node: any): boolean => {
      ...
      const tagMatch =
        !activeTags ||
        activeTags.size === 0 ||
        (node.topics || node.meta?.topics || []).some((t: string) => activeTags.has(t))
