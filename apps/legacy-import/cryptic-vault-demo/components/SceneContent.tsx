...
export default function SceneContent({
  ...
  _enrichedImages: enrichedImages,
  interactionState,
  highlightState,
  _highlightActiveTime: highlightActiveTime,
}: SceneContentProps) {
  if (nodes.length > 0) {
    console.log(
      `[Step D.1] Is node c_001 frozen on RECEIVING props?`,
      Object.isFrozen(nodes.find((n) => n.id === 'c_001'))
    )
  }
  ...
  return (
    <>
      ...
      {viewMode === 'nodes' && interactionState.masterGraphData?.nodes?.length > 0 && (
        <CrypticAnimusScene
          ...
          currentInteractionMode={interactionState.currentInteractionMode || 'mouse'}
      ...
      {viewMode === 'clusters' && <ClusterVisualization nodes={nodes} opacity={1} visible={true} />}

      {/* Brain View: Brain mesh visualization */}
      {viewMode === 'brain' && <BrainMeshView nodes={nodes} opacity={1} visible={true} />}
